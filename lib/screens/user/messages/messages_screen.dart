...
// Message Model with additional properties
class Message {
  final String id;
  final String title;
...
class MessageAttachment {
  final String type; // 'image', 'file', 'location', etc.
  final String url;
...
// Message Provider for state management
class MessageProvider with ChangeNotifier {
  ...
  void archiveMessage(String messageId) {
    ...
    // In a real app, you might move this to an archived list
...
class _MessagesScreenState extends State<MessagesScreen> with SingleTickerProviderStateMixin {
  ...
  String _formatMessageDate(DateTime date) {
    ...
    if (messageDate == today) {
      return DateFormat.jm().format(date); // Format as 3:30 PM
    } else if (messageDate == yesterday) {
      return 'Yesterday';
    } else if (now.difference(date).inDays < 7) {
      return DateFormat.E().format(date); // Format as Mon, Tue, etc.
    } else {
      return DateFormat.MMMd().format(date); // Format as Jan 5
    }
  }
}
...
class _ChatScreenState extends State<ChatScreen> {
    ...
    if (widget.message.isOnline) {
      Future.delayed(const Duration(seconds: 2), () {
        if (mounted) {
          ...
          // After 3 more seconds, send an automated response
  ...
  void _sendMessage() {
    ...
    // If this is a support conversation, simulate a response
    if (widget.message.category == 'Support') {
      ...
      Future.delayed(const Duration(seconds: 1), () {
        if (mounted) {
          ...
          // After 2 more seconds, send an automated response
          Future.delayed(const Duration(seconds: 2), () {
            if (mounted) {
              setState(() {
                _isTyping = false;
